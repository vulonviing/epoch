"""Registry-keyed D1 active and archive records."""
...
def load_data_request(registry_id: str) -> dict[str, Any] | None:
    record = load_active(registry_id)
    return record.get("payload", {}).get("data_request") if record else None
    ...
    *,
    ...
    return _write(
        ...
        _record(
            ...
            payload,
            status="active",
            reason="D1 data execution completed from approved handoff",
            run_id=run_id,
...
def _record(
    ...
    *,
    ...
    return {
        "schema_version": "1",
        "artifact_id": f"d1-{uuid.uuid4().hex[:12]}",
        "agent_id": "D1",

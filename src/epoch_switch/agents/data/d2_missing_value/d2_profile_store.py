"""Registry-keyed D2 active and archive records."""
...
def load_verdict(registry_id: str) -> dict[str, Any] | None:
    """Convenience accessor — returns the stored quality verdict dict or None."""
    record = load_active(registry_id)
    return record.get("payload", {}).get("verdict") if record else None
    ...
    *,
    ...
    return _write(
        ...
        _record(
            ...
            payload,
            status="active",
            reason="D2 data-quality preflight completed from approved handoff",
            run_id=run_id,
...
def list_archive(registry_id: str) -> list[dict[str, Any]]:
    ...
    return (
        [_load(path) for path in sorted(directory.glob("*.json"))]
        if directory.exists()
        else []
    )
...
def _record(
    ...
    *,
    ...
    return {
        "schema_version": "1",
        "artifact_id": f"d2-{uuid.uuid4().hex[:12]}",
        "agent_id": "D2",

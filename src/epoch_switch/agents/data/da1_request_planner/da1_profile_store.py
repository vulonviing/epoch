"""Registry-keyed DA1 active and archive records."""
...
def load_approved_scope(registry_id: str) -> dict[str, Any] | None:
    record = load_active(registry_id)
    return record.get("payload", {}).get("approved_mapping_scope") if record else None
...
def _record(
    ...
    *,
    ...
    return {
        "schema_version": "1",
        "artifact_id": f"da1-{uuid.uuid4().hex[:12]}",
        "agent_id": "DA1",

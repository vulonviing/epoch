"""Registry-keyed TS1 active and archive records.

Verbatim copy of cp1_profile_store.py with agent_id="TS1", artifact prefix "ts-",
and reason strings updated.  Same envelope layout as every other agent store.
"""
...
def promote_active(
    ...
    *,
    ...
    return _write(
        ...
        _record(registry_id, payload, "active", "TS1 topology selection accepted", run_id, review_round, input_refs, llm_provenance),
    )
...
def archive_candidate(
    ...
    *,
    ...
    return _write_archive(
        registry_id,
        _record(registry_id, payload, status, reason, run_id, review_round, input_refs, llm_provenance),
    )
...
def _record(registry_id, payload, status, reason, run_id, review_round, input_refs, llm_provenance=None):
    ...
    return {
        "schema_version": "1",
        "artifact_id": f"ts-{uuid.uuid4().hex[:12]}",
        "agent_id": "TS1",

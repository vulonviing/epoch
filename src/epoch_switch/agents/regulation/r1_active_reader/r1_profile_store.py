"""Registry-keyed R1 active and archive records."""
...
def load(registry_id: str) -> dict[str, Any] | None:
...
def promote_active(
    ...
    *,
    ...
    record = _record(
        ...
        payload,
        status="active",
        reason="DA1 mapping scope approved by human",
        run_id=run_id,
    ...
    return _write(PROFILE_DIR / registry_id / "active.json", record)
...
def save_validation_attempt(
    ...
    *,
    ...
    """Write a repair-audit record to archive (mirrors r2_profile_store pattern)."""
    record = {
        "schema_version": "1",
        "artifact_id": f"r1-{uuid.uuid4().hex[:12]}",
        "agent_id": "R1",
    ...
    return _write_archive(registry_id, record)

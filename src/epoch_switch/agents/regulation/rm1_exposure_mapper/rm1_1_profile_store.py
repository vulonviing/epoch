"""Registry-keyed RM1.1 active and archive records.

Verbatim copy of cp1_profile_store.py with agent_id='RM1.1' and artifact
prefix 'rm1-1-'. RM1.1 shares this package with RM1.2 but keeps its own shelf
directory (`rm1_1_registry_profiles/`) -- see AGENTS.md's blind-pass agent
...
PROFILE_DIR = Path(__file__).parent / "rm1_1_registry_profiles"
...
def promote_active(
    ...
    *,
    ...
    return _write(
        ...
        _record(registry_id, payload, "active", "RM1.1 blind exposure match accepted", run_id, review_round, input_refs, llm_provenance=llm_provenance),
    )
...
def _record(registry_id, payload, status, reason, run_id, review_round, input_refs, llm_provenance=None):
    ...
    return {
        "schema_version": "1",
        "artifact_id": f"rm1-1-{uuid.uuid4().hex[:12]}",
        "agent_id": "RM1.1",

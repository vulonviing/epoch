"""Registry-keyed RM1.2 active and archive records.

Verbatim copy of cp1_profile_store.py with agent_id='RM1.2' and artifact
prefix 'rm1-2-'. Access only via this module. The CLI is the only layer
...
def promote_active(
    ...
    *,
    ...
    return _write(
        ...
        _record(registry_id, payload, "active", "RM1.2 exposure mapping accepted", run_id, review_round, input_refs, llm_provenance=llm_provenance),
    )
...
def _record(registry_id, payload, status, reason, run_id, review_round, input_refs, llm_provenance=None):
    ...
    return {
        "schema_version": "1",
        "artifact_id": f"rm1-2-{uuid.uuid4().hex[:12]}",
        "agent_id": "RM1.2",

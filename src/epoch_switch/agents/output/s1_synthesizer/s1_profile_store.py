"""Registry-keyed S1 active and archive records.

Verbatim copy of o1_profile_store.py with agent_id='S1' and artifact prefix 's1-'.
...
def promote_active(
    ...
    *,
    ...
    return _write(
        ...
        _record(registry_id, payload, "active", "S1 synthesis accepted", run_id, review_round, input_refs, llm_provenance=llm_provenance),
    )
...
def _record(registry_id, payload, status, reason, run_id, review_round, input_refs, llm_provenance=None):
    ...
    return {
        "schema_version": "1",
        "artifact_id": f"s1-{uuid.uuid4().hex[:12]}",
        "agent_id": "S1",

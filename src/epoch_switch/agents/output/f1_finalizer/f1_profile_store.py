"""Registry-keyed F1 active and archive records.

Verbatim copy of o2_profile_store.py with agent_id='F1' and artifact prefix 'f1-'.
...
def promote_active(
    ...
    *,
    ...
    return _write(
        ...
        _record(registry_id, payload, "active", "F1 deliverable approved by human", run_id, review_round, input_refs, llm_provenance=llm_provenance),
    )
...
def _record(registry_id, payload, status, reason, run_id, review_round, input_refs, llm_provenance=None):
    ...
    return {
        "schema_version": "1",
        "artifact_id": f"f1-{uuid.uuid4().hex[:12]}",
        "agent_id": "F1",

"""Registry-keyed persona (P4/P5/P6) active and archive records.

A deliberate deviation from the one-store-per-agent convention: since
PersonaAssessorAgent is one class with three ``agent_id`` instances, this
module takes ``agent_id`` as an explicit parameter instead of hardcoding one
(compare with p1_profile_store.py, which is agent-specific).  Shelves are
kept separate per character under ``registry_profiles/<agent_id>/<registry_id>/``
so P4, P5, and P6 never overwrite each other's record for the same case.
...
def promote_active(
    ...
    *,
    ...
    return _write(
        ...
        _record(agent_id, registry_id, payload, "active", f"{agent_id} persona assessment accepted", run_id, review_round, input_refs, llm_provenance=llm_provenance),
    )
